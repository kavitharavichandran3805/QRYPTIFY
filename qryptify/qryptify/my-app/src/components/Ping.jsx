...
export default function Ping() {
  ...
  // Helper: tokenize question into lowercase words
  ...
  const handleSend = () => {
    ...
    // Check which faq question contains the greatest number of matched words from user input
    let bestMatch = null;
    let maxMatches = 0;

    faqAnswers.forEach((faq) => {
      const questionWords = tokenize(faq.question);
      const matches = questionWords.filter(qw => lowerInput.includes(qw)).length;
    ...
    const botResponse = bestMatch && maxMatches > 0
      ? bestMatch.answer
      : "Sorry, I don't have an answer for that. Please contact support for further assistance.";
      ...
      {chatOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl max-w-md w-full shadow-lg flex flex-col overflow-hidden">
            ...
            {/* Chat Messages */}
            <div className="flex-1 overflow-y-full px-6 py-4 space-y-3 max-h-96">
